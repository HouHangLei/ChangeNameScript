...
#旧文件名前缀
old_file_prefix="UOPL"
#新文件名前缀
new_file_prefix="VOABC"

#旧方法名前缀
old_method_prefix="uopl_"
#新方法名前缀
new_method_prefix="voabc_"

#旧资源名前缀
old_resources_prefix="uopl_"
#新资源名前缀
new_resources_prefix="voabc_"

#Assets.xcassets内旧图片资源前缀
old_assets_image_prefix=uopl_
#Assets.xcassets内新图片资源前缀
new_assets_image_prefix=voabc_

#文件夹名旧前缀（不修改文件夹前缀传空）
old_folder_prefix=U_
#文件夹名新前缀
new_folder_prefix=V_
#文件夹名旧前缀（不修改文件夹前缀传空）（同上，字符串类型）
old_folder_prefix_string="U_"
#文件夹名新前缀（同上，字符串类型）
new_folder_prefix_string="V_"
    ...
    do
        ...
        then
            ...
            then
                ...
                if test $old_folder_prefix_string != $""
                then
            ...
            #改名swift和xib后缀文件名
            if [ "${file##*.}"x = "swift"x ] || [ "${file##*.}"x = "xib"x ] || [ "${file##*.}"x = "h"x ] || [ "${file##*.}"x = "m"x ] || [ "${file##*.}"x = "pch"x ]
            then
