...
#旧方法名前缀
old_method_prefix="gtr_"
#新方法名前缀
new_method_prefix="test_"

#旧资源名前缀
old_resources_prefix="gtr_"
#新资源名前缀
new_resources_prefix="test_"

#文件夹名旧前缀（不修改文件夹前缀传空）
old_folder_prefix=G_
#文件夹名新前缀
new_folder_prefix=T_
#文件夹名旧前缀（同上，字符串类型）
old_folder_prefix_string="G_"
#文件夹名新前缀（同上，字符串类型）
new_folder_prefix_string="T_"

#Assets.xcassets内旧图片资源前缀
old_assets_image_prefix=gtr_
...
foreachd() {
    ...
    do
        ...
        then
            ...
            #改名swift和xib后缀文件名
            if [ "${file##*.}"x = "swift"x ] || [ "${file##*.}"x = "xib"x ]
            then
